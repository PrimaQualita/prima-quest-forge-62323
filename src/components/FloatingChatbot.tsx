...
export const FloatingChatbot = () => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <Card className="fixed bottom-4 right-4 md:bottom-6 md:right-6 w-[calc(100vw-2rem)] md:w-96 h-[calc(100vh-6rem)] md:h-[600px] max-h-[600px] shadow-2xl flex flex-col z-50 bg-background">
          <CardHeader className="border-b flex-shrink-0 py-3 md:py-4">
            <div className="flex items-center justify-between">
              <CardTitle className="flex items-center gap-2 text-base md:text-lg">
                <Bot className="w-4 h-4 md:w-5 md:h-5 text-primary" />
                <span className="md:inline">Assistente de Compliance</span>
              </CardTitle>
              <Button
                variant="ghost"
                size="icon"
          ...
          <CardContent className="flex-1 flex flex-col p-0 min-h-0 overflow-hidden">
              ...
              <div className="space-y-3 md:space-y-4">
                {messages.map((message, idx) => (
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[75%] md:max-w-[70%] rounded-lg p-2.5 md:p-3 break-words overflow-wrap-anywhere ${
                        message.role === "user"
                    ...
                    {message.role === "user" && (
                      <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-accent/10 flex items-center justify-center flex-shrink-0 mt-1">
                ...
                {isLoading && (
                  <div className="flex gap-2 md:gap-3">
                    <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-primary/10 flex items-center justify-center mt-1">
                      <Bot className="w-4 h-4 md:w-5 md:h-5 text-primary animate-pulse" />
                    </div>
                    <div className="bg-muted rounded-lg p-2.5 md:p-3">
                      <p className="text-xs md:text-sm text-muted-foreground">Processando...</p>
                    </div>
                  </div>
                )}
            ...
            <div className="p-3 md:p-4 border-t flex-shrink-0 bg-background">
              <div className="flex gap-2">
                ...
                />
                <Button onClick={handleSend} disabled={isLoading || !input.trim()} size="icon" className="flex-shrink-0 h-9 w-9 md:h-10 md:w-10">
