...
serve(async (req) => {
  ...
  try {
    ...
    if (!trainingId || !documentContent) {
      throw new Error('Training ID e conteúdo do documento são obrigatórios');
    }
    ...
    if (!questionsData.questions || questionsData.questions.length < 30) {
      throw new Error(`IA gerou apenas ${questionsData.questions?.length || 0} questões, esperado 30`);
    }
